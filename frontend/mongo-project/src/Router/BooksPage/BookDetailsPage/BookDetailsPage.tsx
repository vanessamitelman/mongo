...
export function BookDetailsPage() {
  ...
  if (book_details_query.data === null || book_details_query.data === undefined)
    return <div>No book with this id</div>;
  return (
    <main>
      ...
      <NavLink to={`/author/${book_details_query.data.authorId}`}>
        {book_details_query.data.authorId}
      </NavLink>
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
        ...
        >
          ...
          <select {...register('authorId')}>
            {authors_details_query?.data?.map((author, index) => (
